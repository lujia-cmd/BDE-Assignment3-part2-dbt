...
coalesce(100.0 * (active_listings - lag(active_listings) over (partition by listing_neighbourhood order by month_date))
/ nullif(lag(active_listings) over (partition by listing_neighbourhood order by month_date),0), 0) as per_change_active,

-- Percentage change for inactive listings 
coalesce(100.0 * ((total_listings - active_listings) - lag(total_listings - active_listings) over (partition by listing_neighbourhood order by month_date))
/ nullif(lag(total_listings - active_listings) over (partition by listing_neighbourhood order by month_date),0), 0) as per_change_inactive,
